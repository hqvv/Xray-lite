...
func TestTCPLocalNameServer(t *testing.T) {
	...
	common.Must(err)
	s, err := InitNameServer(context.Background(), NewTCPLocalNameServer, url, nil, QueryStrategy_USE_IP)
	common.Must(err)
...
func TestTCPLocalNameServerWithIPv4Override(t *testing.T) {
	url, err := url.Parse("tcp+local://8.8.8.8")
	common.Must(err)
	s, err := InitNameServer(context.Background(), NewTCPLocalNameServer, url, nil, QueryStrategy_USE_IP4)
	common.Must(err)
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*5)
	ips, _, err := s.QueryIP(ctx, "google.com", net.IP(nil), dns_feature.IPOption{
		IPv4Enable: true,
...
func TestTCPLocalNameServerWithIPv6Override(t *testing.T) {
	url, err := url.Parse("tcp+local://8.8.8.8")
	common.Must(err)
	s, err := InitNameServer(context.Background(), NewTCPLocalNameServer, url, nil, QueryStrategy_USE_IP6)
	common.Must(err)
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*5)
	ips, _, err := s.QueryIP(ctx, "google.com", net.IP(nil), dns_feature.IPOption{
		IPv4Enable: true,
